...
from networks.layers import GroupNorm2d, GroupNorm3d
...
# 3D convolution layer with group normalization and relu activation
...
class UNetConvBlock3D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, padding='SAME', group_norm=True):
        ...
        if group_norm:
            self.conv1 = ConvGroupNormRelu3D(in_channels, out_channels, kernel_size=kernel_size, padding=padding)
            self.conv2 = ConvGroupNormRelu3D(out_channels, out_channels, kernel_size=kernel_size, padding=padding)
        else:
            self.conv1 = ConvBatchNormRelu3D(in_channels, out_channels, kernel_size=kernel_size, padding=padding)
            self.conv2 = ConvBatchNormRelu3D(out_channels, out_channels, kernel_size=kernel_size, padding=padding)

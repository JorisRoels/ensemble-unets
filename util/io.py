...
from util.tools import load_net, overlay
...
def imwrite3D_overlayed(x, y, dir, prefix='', rescale=False):
    if not os.path.exists(dir):
        os.mkdir(dir)
    for i in range(0,x.shape[0]):
        if rescale:
            img = overlay(x[i, :, :], y[i, :, :], alpha=0.2)
        else:
            img = overlay(x[i, :, :]/255, y[i, :, :], alpha=0.2)
        scipy.misc.imsave(dir + '/' + prefix + str(i) + '.png', (img*255).astype('uint8'))
